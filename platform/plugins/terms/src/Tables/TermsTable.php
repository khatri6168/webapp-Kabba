...
class TermsTable extends TableAbstract
{
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, Terms $terms)
    {
        ...
        if (! Auth::user()->hasAnyPermission(['terms.edit', 'terms.destroy'])) {
    ...
    {
        $data = $this->table
            ->eloquent($this->query()->where('is_global',2))
            ->editColumn('title', function (Terms $item) {
                if (! Auth::user()->hasPermission('terms.edit')) {
                    return $item->title;
                }

                return Html::link(route('terms.edit', $item->getKey()), $item->title);
            })
            ...
            ->addColumn('operations', function (Terms $item) {
                return $this->getOperations('terms.edit', 'terms.destroy', $item);
            });
    ...
    {
        return $this->addCreateButton(route('terms.create'), 'terms.create');
    }

    public function bulkActions(): array
    {
        return $this->addDeleteAction(route('terms.deletes'), 'terms.destroy', parent::bulkActions());
    }
