                    ...
                    <table class="table table-striped">
                        ...
                        @forelse ($payments as $payment)
                            <tr>
                                ...
                                <td class="text-start"><a href="{{ route('orders.edit', $payment->order_id) }}" target="__blank"> {{ $payment->order->code }}</a></td>
                                <td><a href="{{ route('payment.show', $payment->id) }}" target="__blank">{{ $payment->charge_id }}</a></td>
                                <td>{{ $payment->amount }} {{ $payment->currency }}</td>
                                <td>{{ $payment->payment_channel->label() }}</td>
                                <td>{!! $payment->status->toHtml() !!}</td>
                                <td class="text-center" style="width: 120px;">
                                    <a href="{{ route('orders.edit', $payment->order_id) }}" class="btn btn-icon btn-sm btn-info me-1 btn-trigger-edit-payment" data-bs-toggle="tooltip" role="button" data-bs-original-title="{{ trans('core/base::forms.view_new_tab') }}" target="_blank">
