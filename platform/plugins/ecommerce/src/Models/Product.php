...
class Product extends BaseModel
{
    ...
    public function getMachineHoursAttribute(){
        $machineHours = $this->findHours($this->rental_type);
        $machineHours['hourly_tracking'] = (bool)$this->hour_track;
        $machineHours['hourly_rate'] = $this->over_rate;
        // $machineHours['hourly_type'] = $this->findHours($this->rental_type);
        // $machineHours['allocated_hours'] = $this->findHours($this->rental_type);

        return $machineHours;
    }

    public function findHours($type){
        switch ($type) {
            case '1':
                $defaultAllocatedHours = get_ecommerce_setting('daily_hours');
                $hourlyType = "Daily";
                break;
            case '2':
                $defaultAllocatedHours = get_ecommerce_setting('weekend_hours');
                $hourlyType = "Weekend";
                break;
            case '3':
                $defaultAllocatedHours = get_ecommerce_setting('weekly_hours');
                $hourlyType = "Weekly";
                break;
            case '4':
                $defaultAllocatedHours = get_ecommerce_setting('monthly_hours');
                $hourlyType = "Monthly";
                break;
            default:
                $defaultAllocatedHours = 0;
                $hourlyType = "No Rental";
                break;
        }
        $arr['hourly_type'] = $hourlyType;
        $arr['allocated_hours'] = $defaultAllocatedHours;
        return $arr;
    }
    ...
    public function deliveryPickups(): HasMany
    {
        return $this->hasMany(DeliveryPickup::class, 'product_id');
    }
