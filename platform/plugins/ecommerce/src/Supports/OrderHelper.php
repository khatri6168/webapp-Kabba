...
class OrderHelper
{
    ...
    public function handleAddCart(Product $product, Request $request): array
    {
        ...
        if ($request->input('options')) {
            if (isset($product->options)) {
                ...
                if (isset($request->input('options')[$optionId]['values'])) {
                    $options = $this->getProductOptionData($request->input('options'), $parentProduct);
                }

                // if the customer check the delivery option, add delivery option to cart info
                if ($request->input('delivery')) {
                    $deliveryOption = [
                        'option_value' => 'Delivery',
                        'affect_price' => $product->delivery_price,
                        'affect_type' => 0,
                        'option_type' => 'checkbox',
                    ];
                    $options['optionCartValue'][key($request->input('options'))][] = $deliveryOption;
                }
                // if the customer check the pickup option, add pick option to cart info
                if ($request->input('pickup')) {
                    $pickupOption = [
                        'option_value' => 'Pick Up',
                        'affect_price' => $product->delivery_price,
                        'affect_type' => 0,
                        'option_type' => 'checkbox',
                    ];
                    $options['optionCartValue'][key($request->input('options'))][] = $pickupOption;
                }
            }
        }
    ...
    public function getProductOptionData(array $data, $product = []): array
    {
        $result = [];
        if (! empty($data)) {
            if (isset($data['options']['optionCartValue'])) {
                $optionKey = array_keys($data['options']['optionCartValue'])[0];
                return $data['options'];
            }

            if (count($data) > 1) {
                foreach ($data as $key => $option) {
                    if (empty($option) || !is_array($option) || !isset($option['values'][0])) {
                        continue;
                    }
                    // echo $option['values'][0]."\n";

                    $optionKey = $key;

                    $optionValue = OptionValue::select(['option_value', 'affect_price', 'affect_type'])->where('option_id', $optionKey);
                    ...
                    $result['optionCartValue'][$optionKey] = $optionValue->get()->toArray();

                    if (isset($option['option_type'])) {
                        foreach ($result['optionCartValue'][$optionKey] as &$item) {
                            $item['option_type'] = $option['option_type'];
                        }
                        // dd($result['optionCartValue'][$optionKey]);
                ...
                foreach ($data as $key => $option) {
                    ...
                    if (count($option) == 1) {
                        ...
                        if (isset($option['value'])) {
                            $optionValue->whereIn('option_value', [0 => $option['value']]);
                        }
                    }
    ...
    public function getProductOptionDataNew(array $data): array
    {
        $result = [];
        if (!empty($data)) {
            $result = [];

            $optionKey = array_keys($data)[0];
            foreach ($data as $key => $option) {
                ...
                if (isset($option['option_type'])) {
                    if ($option['option_type'] != 'field' && isset($option['values'])) {
                        ...
                        $optionValue->whereIn('option_value', [0 => $option['option_value']]);
                ...
                if (isset($option['option_value'])) {
                    $result['optionCartValue'][$key] = $optionValue->get()->toArray();
                    foreach ($result['optionCartValue'][$key] as $index => &$item) {
                        $item['option_type'] = $option[$index]['option_type'] ?? '';
                    }
    ...
    protected function storeOrderBillingAddress(array $data, array $sessionData = [])
    {
        ...
        if ($orderId) {
            ...
            // if ($billingAddressSameAsShippingAddress == 1) {
            //     $addressData = Arr::only($sessionData, ['first_name','last_name','name', 'phone', 'email', 'country', 'state', 'city', 'address', 'zip_code']);
            ...
            //     $billingAddressData['type'] = OrderAddressTypeEnum::SHIPPING;
            ...
            if(Arr::get($sessionData, 'is_save_order_shipping_address', true) == true){
                if($billingAddressSameAsShippingAddress == 1){
                    $addressData = Arr::only($sessionData, ['first_name','last_name','name', 'phone', 'email', 'country', 'state', 'city', 'address', 'zip_code']);
                    ...
                    $baddressData = Arr::get($sessionData, 'billing_address', []);

                    if(!empty($baddressData) && $baddressData['first_name'] == ''){
                       // dd($baddressData);
                        $addressData = Arr::only($sessionData, ['first_name','last_name','name', 'phone', 'email', 'country', 'state', 'city', 'address', 'zip_code']);
                    ...
                    $addressData = Arr::get($sessionData, 'billing_address', []);
                    $billingAddressData = $addressData;
                    $billingAddressData['order_id'] = $orderId;
                    $billingAddressData['type'] = OrderAddressTypeEnum::BILLING;
                    app(OrderAddressInterface::class)
