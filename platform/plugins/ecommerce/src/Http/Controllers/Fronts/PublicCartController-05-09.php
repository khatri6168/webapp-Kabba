...
class PublicCartController extends Controller
{
    ...
    public function store(CartRequest $request, BaseHttpResponse $response)
    {
        ...
        if ($request->has('rawId') && !empty($request->get('rawId'))) {

            $cartItem = Cart::instance('cart')->get($request->input('rawId'));
            ...
            if (!empty($cartItem)) {
                ...
                if ($product) {
                    ...
                    $product->quantity = (int)$product->quantity - (int)Arr::get($request->all(), 'qty', 0) + 1;
                    ...
                    if ($product->isOutOfStock()) {
                        Cart::instance('cart')->update($request->input('rawId'), Arr::get($request->all(), 'options'));
                        $outOfQuantity = true;
                    } else {
                        Cart::instance('cart')->update($request->input('rawId'), Arr::get($request->all(), 'options'));
                        Cart::instance('cart')->update($request->input('rawId'), Arr::get($request->all(), 'qty'));
                    }
                    return $response
                        ->setData([
            ...
            // Cart::instance('cart')->remove($request->input('rawId'));
        }
