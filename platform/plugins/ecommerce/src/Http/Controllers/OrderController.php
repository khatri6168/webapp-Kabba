...
class OrderController extends BaseController
{
    ...
    public function store(CreateOrderRequest $request, BaseHttpResponse $response)
    {
        ...
        if ($order) {
            ...
            foreach (Arr::get($data, 'products') as $productItem) {
                ...
                // get order address from delivery_pickup
                $deliveryData = [];
                $oldDelivery = $this->deliveryPickUpRepository->getFirstBy(['order_id' => $order->id, 'product_id' => $product->id]);

                if ($oldDelivery) {
                    $deliveryData = [
                        'name' => $oldDelivery['name'],
                        'phone' => $oldDelivery['phone'],
                        'email' => $oldDelivery['email'],
                        'country' => $oldDelivery['country'],
                        'state' => $oldDelivery['state'],
                        'city' => $oldDelivery['city'],
                        'address' => $oldDelivery['address'],
                        'order_id' => $order->id,
                        'zip_code' => $oldDelivery['zipcode'],
                        'comment' => '',
                        'product_id' => $product->id,
                    ];
                } else {
                    $orderAddress = $this->orderAddressRepository->getFirstBy(['order_id' => $order->id, 'type' => 'shipping_address']);
                ...
                $deliveryData['delivery_date'] = $deliveryDate;
                ...
                if ($product->rental_type == RENTAL_TYPE_DAILY) {
                    $pickDate = date('Y-m-d', strtotime( "+$duration day"));
                } else if ($product->rental_type == RENTAL_TYPE_WEEKEND) {
                    $duration = $duration * 3;
                    $pickDate = date('Y-m-d', strtotime("+$duration days"));
    ...
    public function edit(int|string $id)
    {
        ...
        $delivery = $this->deliveryPickUpRepository->getFirstBy(['order_id' => $order->id]);
        if (!empty($delivery)) {
        ...
        return view('plugins/ecommerce::orders.edit', compact('order', 'weight', 'defaultStore', 'delivery', 'readonly'));
    }

    public function update(int|string $id, UpdateOrderRequest $request, BaseHttpResponse $response)
    {
        if ($request->input('operation') == 'machine_hours') {
            $machineHours = $request->input('machineHour');
            for ($i = 0; $i < count($machineHours); $i++) {
                $this->orderMachineHourRepository->createOrUpdate($machineHours[$i],
                    [
                        'order_id' => $machineHours[$i]['order_id'],
                        'product_id' => $machineHours[$i]['product_id']
                    ]);
            }
    ...
    public function machineHour(int|string $id, UpdateOrderRequest $request, BaseHttpResponse $response)
    {
        $order = $this->orderRepository->findOrFail($id);
        $order->fill($request->input());
    ...
    public function updateImage(Request $request, BaseHttpResponse $response) {
        $order = $this->orderRepository->findOrFail($request->input('orderid'));
    ...
    public function loginCustomer(int|string $id) {
...
//        $customer->dob = Carbon::parse($request->input('dob'))->toDateString();
    ...
    public function updateByOrderId(int|string $id, Request $request, BaseHttpResponse $response) {
        ...
        $this->deliveryPickUpRepository->update(['order_id' => $id], $data);
        return $response->setMessage('Updated Successfully');
    }
}
